...
public class BatteryDataMarker {
     ...
     * @param blankWindows     {@link MarkedDataPoints} None of the windows shall be marked.
     */
                ...
                if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime) {
                    if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime
                        ...
                        if (temp > 20) {
                            ...
                            if (rawBatteryLevels.get(j).getValue() <= Integer.parseInt(Config.get("PHONE_BATTERY_DOWN"))) {
                                blankWindows.get(i).setQuality(Integer.parseInt(Config.get("PHONE_BATTERY_DOWN_LABEL")));
                                temp2 = 1;
                                break;
                            } else {
                                blankWindows.get(i).setQuality(Integer.parseInt(Config.get("PHONE_POWERED_OFF_LABEL")));
                                temp2 = 1;
                                break;
                            }
                        }
                ...
                if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime) {
                    ...
                    if (markedWindows.get(i).getQuality() == 999) {
                        if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime
                            ...
                            if (voltageValue <= Integer.parseInt(Config.get("AUTOSENSE_BATTERY_DOWN"))) {
                                markedWindows.get(i).setQuality(Integer.parseInt(Config.get("SENSOR_BATTERY_DOWN")));
                                break;
                            } else {
                                markedWindows.get(i).setQuality(Integer.parseInt(Config.get("SENSOR_POWERED_OFF")));
                                break;
                            }
                        }
                    }
                ...
                if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime) {
                    if (markedWindows.get(i).getQuality() == 999) {
                        if (rawBatteryLevels.get(j).getStartTimestamp() >= startTime
                            ...
                            // mark blankWindows as sensor battery down/off
                            if (rawBatteryLevels.get(i).getValue() <= Integer.parseInt(Config.get("MOTIONSENSE_BATTERY_DOWN"))) {
                                markedWindows.get(i).setQuality(Integer.parseInt(Config.get("SENSOR_BATTERY_DOWN")));
                                break;
                            } else {
                                markedWindows.get(i).setQuality(Integer.parseInt(Config.get("SENSOR_POWERED_OFF")));
